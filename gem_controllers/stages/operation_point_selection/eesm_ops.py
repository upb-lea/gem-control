...
class EESMOperationPointSelection(FieldOrientedControllerOperationPointSelection):
    ...
    def tune(self, env, env_id, current_safety_margin=0.2):
        ...
        self.torque_equation = lambda i_d, i_q, i_e: 3 / 2 * self.p * (
                    self.l_m * i_e + (self.l_d - self.l_q) * i_d) * i_q

        self.loss = lambda i_d, i_q, i_e: np.abs(i_d) * self.r_s + np.abs(i_q) * self.r_s + np.abs(i_e) * self.r_e

        self.poly = lambda i_e, psi, torque: [self.l_d ** 2 * (self.l_d - self.l_q) ** 2,
                                              2 * self.l_d ** 2 * (
                                                      self.l_d - self.l_q) * self.l_m * i_e + 2 * self.l_d * self.l_m * i_e * (
                                                      self.l_d - self.l_q) ** 2,
                                              self.l_d ** 2 * (self.l_m * i_e) ** 2 + 4 * self.l_d * (
                                                          self.l_m * i_e) ** 2 * (self.l_d - self.l_q) + (
                                                          (self.l_m * i_e) ** 2 - psi ** 2) * (
                                                          self.l_d - self.l_q) ** 2,
                                              2 * self.l_q * (self.l_m * i_e) ** 3 + 2 * (
                                                          (self.l_m * i_e) ** 2 - psi ** 2) * self.l_m * i_e * (
                                                          self.l_d - self.l_q),
    ...
    def _calculate_luts(self):
        ...
        self.t_max_psi = sp_interpolate.interp1d(np.linspace(0, self.psi_max, self.psi_count), 0.99 * self.t_max_psi, kind='linear')
        ...
        self.t_grid, self.psi_grid = np.mgrid[
                                     0: self.t_max: np.complex(0, self.t_grid_count),
                                     0: self.psi_max: np.complex(self.psi_grid_count)
                                     ]

        self.i_d_inter = sp_interpolate.griddata((best_params_psi[:, 0], best_params_psi[:, 1]), best_params_psi[:, 2],
                                                 (self.t_grid, self.psi_grid), method='linear')
        self.i_q_inter = sp_interpolate.griddata((best_params_psi[:, 0], best_params_psi[:, 1]), best_params_psi[:, 3],
                                                 (self.t_grid, self.psi_grid), method='linear')
