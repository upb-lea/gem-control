...
def shunt_dc_cc(emf_feedforward):
    """
    Args:
        emf_feedforward: Boolean whether emf feedforward stage is included

    Returns:
        Function to build the Shunt DC Output Current Control Block
    """

    def cc_shunt_dc(start, control_task):
        """
        Function to build the Shunt DC Output Current Control Block
        Args:
        ...
        if control_task == 'CC':
            ...
            Connection.connect(start, add_current.input_left[0], text=r'$i^{*}_{\mathrm{a}}$', text_align='left',
                               text_position='start')
        ...
        # Inputs of the stage
        inputs = dict(i_ref=[add_current.input_left[0], dict(text=r'$i^{*}_{\mathrm{a}}$')],
                      i=[add_current.input_bottom[0], dict(text=r'-', move_text=(-0.2, -0.2), text_position='end',
                                                           text_align='right')])
        ...
        if emf_feedforward:
            ...
            box_psi = Box(add_emf.position.sub_y(2.5), size=(1, 0.8), text=r"$L'_{\mathrm{e}} i_{\mathrm{e}}$",
                          inputs=dict(bottom=1), outputs=dict(top=1))

            # Connection between the multiplication and add block
            Connection.connect(box_psi.output_top, add_emf.input_bottom, text=r'$u^{0}$',
                               text_position='end', text_align='right', move_text=(-0.1, -0.2))
