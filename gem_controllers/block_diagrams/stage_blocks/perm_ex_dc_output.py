...
def perm_ex_dc_output(emf_feedforward):
    ...
    def _perm_ex_dc_output(start, control_task):
        ...
        # Perm Ex DC motor block
        dc_perm_ex = DcPermExMotor(converter.position.sub_y(3), size=1.2, input='top', output='left')

        # Connections between converter and motor block
        conv_motor = Connection.connect(converter.output_bottom, dc_perm_ex.input_top, text=['', r'$i$'],
        ...
        if emf_feedforward or control_task in ['SC']:
            ...
            con_omega = Connection.connect(dc_perm_ex.output_left[0].sub_x(2), dc_perm_ex.output_left[0],
